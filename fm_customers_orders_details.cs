...
namespace BasicMRP
{
    public partial class fm_customers_orders_details : Form
    {
        #region values
        ...
        #region critics
        public fm_customers_orders_details()
        {
            InitializeComponent();
        }

        private void fm_customers_orders_details_Load(object sender, EventArgs e)
        {
            lbSelectedO.Text = "Seçili Sipariş ID: " + fm_customers_orders.id;
            itemid = -1;
        ...
        private void txtOdAmount_Enter(object sender, EventArgs e)
        {
        ...
        private void txtOdAmount_Leave(object sender, EventArgs e)
        {
            if (txtOdAmount.Text == "")
            {
        ...
        private void fillproduct2()
        {
            ...
            JOIN item ON sales_order_detail.itemid = item.id WHERE orderid = " + fm_customers_orders.id, conn);
            DataTable dt = new DataTable();
            SqlDataAdapter da = new SqlDataAdapter(fill);
            da.Fill(dt);
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                if (itemid != -1 && itemtype != "HM" && (txtOdAmount.Text != "" && txtOdAmount.Text != "Sipariş Miktarı"))
                {
                    SqlCommand additemtoorder = new SqlCommand("INSERT INTO sales_order_detail(orderid, itemid, amount, unitcode)" +
                    "VALUES(@oid, @itmid, @amo, @ucode)", conn);
                    additemtoorder.Parameters.AddWithValue("@oid", SqlDbType.Int).Value = fm_customers_orders.id;
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                if (orderdetailregistryid != -1)
                {
                    ...
                    SqlCommand deleteitemfromorder = new SqlCommand("DELETE FROM sales_order_detail WHERE id=@id", conn);
